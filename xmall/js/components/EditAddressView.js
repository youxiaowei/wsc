define(['./BaseView', '../models/AddressDetailModel', 'require', '../libs/cityselector/jquery.cxselect', '../loader'], function(BaseView, addressModel, require, ct, loader) {
  return BaseView.extend({
    ...
    events: {
      ...
      "click #selectP":"selectP",
      "click #selectCity":"selectCity",
      "click #selectArea":"selectArea"
    },
    ...
    onEditAddr: function(data) {
      ...
      _this.sendData(window.app.api + "/my-address/getProvinceList", {}, function(res1) {
        if (res1.status == '0') {
          application.setLocalStorage('provinceList', res1.data);
          var html = '';
      ...
      $("#city select[name='address.idP']").bind('change', function(e, firstload) {
        _this.$el.find(".valueP").val( _this.$el.find(".province  option:selected").text());
       
        _this.sendData(window.app.api + "/my-address/getCityList", {
          pid: e.target.value
        }, function(res2) {
          if (res2.status == '0') {
            application.setLocalStorage('cityList', res2.data);
      ...
      $("#city select[name='address.idCity']").bind('change', function(e, firstload) {
        _this.$el.find(".valueCity").val( _this.$el.find(".city  option:selected").text());
       
        _this.sendData(window.app.api + "/my-address/getCityList", {
          pid: e.target.value
        }, function(res2) {
          if (res2.status == '0') {
            application.setLocalStorage('areaList', res2.data);
            var html = '<option value=-1>请选择</option>';
            for (var i = 0; i < res2.data.length; i++) {
              html += "<option value=" + res2.data[i].id + ">" + res2.data[i].name + "</option>";
            }
      ...
      $("#city select[name='address.idArea']").bind('change', function(e) {
          _this.$el.find(".valueArea").val( _this.$el.find(".area  option:selected").text());
      ...
      $("#city select[name='address.idP']").bind('change', function(e) {
        ...
        _this.sendData(window.app.api + "/my-address/getCityList", {
          ...
          if (res2.status == '0') {
            ...
            $("#city select[name='address.idArea']").append(tmp);
            $("#city select[name='address.idCity']").append(html);
            _this.$el.find(".valueCity").val("");
            _this.$el.find(".valueArea").val("");
          }
        });
      });
    ...
    selectP:function(){
     var type = 1;
      Backbone.history.navigate(
          '#my-navigate/selectAddress?type=' + type, {
              trigger: true
              
          });
    },
     selectCity:function(){
       var city=this.$el.find(".province  option:selected").val();
       if(city && city != "-1"){
          var type = 2;
          Backbone.history.navigate(
              '#my-navigate/selectAddress?type=' + type, {
         ...
         library.Toast('请先选择省份');
      }
      
    },
    selectArea:function(){
      var area=_this.$el.find(".city  option:selected").val();
      if(area && area != "-1"){
        var type = 3;
        Backbone.history.navigate(
        '#my-navigate/selectAddress?type=' + type, {
