...
define(['require', './PageView'], function(require, PageView) {
  ...
  return PageView.extend({
    ...
    btnRegisterClick: function() {
      if ($('.reg-input-account').val().trim().length < 1) {
      ...
      } else if (this.$('#register_input').val().length < 1) {
        ...
        this.sendData("/verifyCheckCode", {
          userPhone: userPhone,
          checkCode: $('#register_input').val(),
          sessionId: this.sessionid
        }, function(res) {
          if (res.status == "0") {
            library.DismissLoadingBar();
            Backbone.history.navigate('#my-navigate/register-pwd?userPhone=' + userPhone + "&sessionId="+res.data.sessionId, {
              trigger: true
            });
          } else {
            library.DismissLoadingBar();
